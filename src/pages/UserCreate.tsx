...
import { createAsyncUser } from "../store/users"



export default function UsersCreates() {
    ...
    const { isLoading } = useSelector((store: StoreType) => store.users)
    

    const createUser = () => {
        if (email) {
            dispatch(createAsyncUser({
                email, role, id: 311, age: 25,
                address: { state: "", address: "", city: "" },
                firstName: "ali", lastName: "vali", password: "123456",
                gender:"female" , phone:"987454632" , username:"mali",
                image:"" , country:"brazil"
            }))
        }
    }
            ...
            <div className="flex flex-wrap gap-y-5 mt-8">
                <div className="w-full lg:w-[24%]">
                    <div className="bg-dark-800 p-5 rounded-xl space-y-5">
                        ...
                        <p className="text-lg font-medium line-clamp-2">{"FirstName"}</p>
                        <p>LastName: <span className="font-semibold ml-1 text-lg">{"LastName"}</span></p>
                        <p>Email: <span className="font-semibold ml-1 text-sm">{email || "Email"} </span></p>

                        <div className="flex gap-2">
                            <Button onClick={createUser} variant={"outline"} >Save User</Button>
