...
import { editAsyncCategory, getSingelCategory } from "../store/category"
...
export default function CategoryEdit() {
    ...
    const { id = "fashion" } = useParams()

    const dispatch: DispatchType = useDispatch()

    const { singleCategory, isLoading } = useSelector((store: StoreType) => store.categories)


    useEffect(() => {
        dispatch(getSingelCategory(id))
    }, [id])

    const editCategory = () => {
        if (title && slug) {
            dispatch(editAsyncCategory({
                ...singleCategory as CategoryType,
                "name": title,
                "slug": slug,
            }))
        }
            ...
            <div className="flex flex-wrap gap-y-5 mt-8">
                <div className="w-full lg:w-[24%]">
                    <div className="bg-dark-800 p-5 rounded-xl space-y-5">
                        ...
                        <p className="text-lg font-medium line-clamp-2">{singleCategory?.name || "Category Title"}</p>
                        <p>Created By: <span className="font-semibold ml-1 text-lg">Masoud Bahamin </span></p>
                        <p>Name: <span className="font-semibold ml-1 text-lg">name </span></p>
                        <p>Slug: <span className="font-semibold ml-1 text-lg">{singleCategory?.slug || "Category Slug"} </span></p>

                        <div className="flex gap-2">
                            <Button onClick={editCategory} variant={"outline"} >Save Product</Button>
